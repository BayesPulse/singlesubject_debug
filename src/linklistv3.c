  ...
  for (i = 0; i < iter; i++) {
    ...
    if (!(i%NNN)) {
      ...
      printf("mu %.2lf A %.2lf s %.2lf d %.4lf  v %.4le\n", 
             ...
             parms->sigma);
      printf("pmdvar00 %.2lf pmdvar11 %.2lf pmdvar01 %.2lf \n",
             pmd_var[0][0], pmd_var[1][1], pmd_var[0][1]);
      print_list(list);
      printf("pct rem = %.2lf pct rew = %.2lf pct time = %.2lf\n",
             (double)arem/(double)nrem, 
             (double)arew/(double)nrew,
             (double)atime/(double)ntime);
      printf("pct md = %.2lf revm = %.2lf revw = %.2lf\n", 
...
void draw_fixed_effects(Node_type *list, 
  ...
  for (j = 0; j < 2; j++) {
    ...
    //----DEBUGGING----//
    //if (j == 1) {
    //printf("mean of gibbs draw for width posterior = %f\n", gmean);
    //printf("SD of gibbs draw for width posterior = %f\n", sqrt(gvar));
    //}
    //----DEBUGGING----//
...
void draw_re_sd(Node_type *list, 
  ...
  //printf("current sd = %f\n", parms->re_sd[1]);
  //printf("proposed new sd = %f\n", new_sd[1]);
  //printf("sd for width proposal = %f\n", v2);
  //------- DEBUGGING---------//
  ...
  for (j = 0; j < 2; j++) {
    ...
    while (node != NULL) {
      ...
      //if (j == 1) {
      //  printf("node %d\n", npulse);
      //  printf("pulse width = %0.20f\n", node->theta[j]);
      //  printf("mean width = %0.20f\n", parms->theta[j]);
      //}
      //------- DEBUGGING---------//
      npulse++;
      node  = node->succ;
    ...
    //  //printf("node->theta - parms->theta = %f\n", debug1);
    //  printf("psum (sum of squared diffs) = %.20f\n", psum);
    //  printf("npulses = %d\n", npulse);
    //}
    //------- DEBUGGING---------//
    ...
    if (new_sd[j] > 0 && new_sd[j] < priors->re_sdmax[j]) {
      ...
      //------- DEBUGGING---------//
      //if (j == 1) {
      ////  printf("new_sd[1]=%f is within U(0,re_sdmax)\n", new_sd[j]);
      //  printf("re_sd[1]=%.20f\n", parms->re_sd[j]);
      //  printf("prop_old = %.20f\n", 1.0/(parms->re_sd[j] * parms->re_sd[j]));
      //  printf("prop_new = %.20f\n", 1.0 / (new_sd[j] * new_sd[j]));
      //  printf("prop_ratio1 = %.20f\n", psum * 0.5 * (prop_old - prop_new));
      //  printf("prop_ratio2 = %.20f\n", prop_ratio);
      //}
      //------- DEBUGGING---------//
