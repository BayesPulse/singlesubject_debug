...
int main(int argc, char *argv[]) {
  ...
  for (a=0;a<1;a++) {
    ...
    printf("Welcome to decon.\n");
    printf("You are using version %s\n", VERSION);
    printf("\n\n");
    ...
    if (finput == NULL) {
      ...
      fprintf(stderr, "Value of argument: %s\n", argv[1]);
      exit(EXIT_FAILURE);
    }
    ...
    printf("Argument file used:\n");
    printf("%s \n", argv[1]);
    printf("Contents of argument file:\n");
    printf("%s \n", datafile);
    printf("%s \n", common1);
    printf("%s \n", parm1);
    printf("%lu %lu %lu\n", seed[0], seed[1], seed[2]);
    ...
    ///*{{{*/
    if (priorgamma < -0.001) {
      printf("Using the %d-order statistic prior.\n", mmm);

    } else if (priorgamma > 1.001) {

      printf("Invalid value used for priorgamma in Strauss.\n");
      printf("priorgamma = %f", priorgamma); 
      printf("Error occurred prior to running mcmc\n");
      exit(EXIT_FAILURE);
      
    } else {

      printf("Using the Strauss prior with gamma = %f\n", priorgamma);

      if (fabs(priorgamma) < EPS && priorgamma != 0) {
        printf("WARNING: Hard Core attempted, but priorgamma NOT");
        printf("exactly equal to 0!!\n");
